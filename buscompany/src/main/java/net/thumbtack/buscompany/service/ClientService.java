...
import net.thumbtack.buscompany.config.KafkaConsumerPlace;
...
public class ClientService extends ServiceBase {
    ...
    public RegisterClientDtoResponse registerClient(RegisterClientDtoRequest request)
        ...
        RegisterClientDtoResponse registerClientDtoResponse = new RegisterClientDtoResponse(res.getId(),
                res.getSurname(),
    ...
    public UpdateClientDtoResponse updateClient(long id, UpdateClientDtoRequest request)
            throws ServiceException {
        ...
        return new UpdateClientDtoResponse(client.getSurname(), client.getName(),
    ...
    public ChoosePlaceResponse choosePlace(ChoosePlaceRequest request) throws ServiceException {
        ...
        for (Passenger pas : passengers) {
            if (pas.getFirstName().equals(request.getFirstName()) && pas.getLastName()
                    .equals(request.getLastName()) && pas.getPassport() == request.getPassport()) {
                flag = true;
        ...
        Place place = null;
        int index = Collections.binarySearch(places, new Place(request.getPlace()),
                Comparator.comparingInt(Place::getNumber));
        ...
        return new ChoosePlaceResponse(request.getOrderId(), ticket, request.getLastName(),
                request.getFirstName(), request.getPlace());
    }
