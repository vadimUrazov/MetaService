...
public class ClientDaoImpl extends BaseDaoImpl implements ClientDao {
    ...
    @Override
    @Transactional
    public Place choosePlace(Passenger passenger, int place, int car, Order order) throws ServiceException {
        LOGGER.debug("DAO create order{ }");
        var res = new Place(place, car);
        var idDay = order.getDayTrip().getId();

        String sql = "UPDATE place SET id_passenger=:passenger WHERE  number=:place AND car=:car AND id_day_trip=:day AND place.id_passenger IS NULL";
        String free = "UPDATE place SET id_passenger=null WHERE id_day_trip=:day AND number=:place AND car=:car";

        var placeCount = entityManager.createNativeQuery(sql, Place.class)
                .setParameter("passenger", passenger.getId()).setParameter("place", place).setParameter("car", car).setParameter("day", idDay).executeUpdate();

        var oldPlace = passenger.getPlace();
        if (placeCount == 1 && oldPlace != null) {
            entityManager.createNativeQuery(free).setParameter("day", idDay).setParameter("place", oldPlace.getNumber()).setParameter("car", car);
        }
